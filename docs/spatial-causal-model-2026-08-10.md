...
## Explicit bidirectional probe collector

The policy-dependent graph target was replaced at the data-collection boundary
with an optional save-state experiment. Starting from each verified endpoint,
the collector tests every configured controller action at the original branch
duration. At deeper levels it retains a bounded beam of the closest outcomes.
Every path is compared with a matched control generated by restoring the
original root and applying NOOP for the same total emulator frames. This makes
the label robust to autonomous animation without introducing a player, object,
hazard, coordinate, or room detector.

The first strict Room 1 collection probed six branches and 42 paths. All six
returned at depth 1. Directional inverse paths had matched-NOOP L1 between
0.00106 and 0.00153, while the non-return path cluster began around 0.00267.
The first strict Room 2 collection probed nine branches and 63 paths; all nine
also returned at depth 1. These are useful positive observations but not a
balanced training set.

The strict Room 2 trajectory was then extended for 12 decisions. It probed 108
branches and 868 paths: 104 branches returned at depth 1, two at depth 2, and
two distinct pixel states had no return within the configured budget. Both
non-returns followed `RIGHT,16`, exhausted 35 paths, and remained at least
0.003287 from their matched control. The run balanced 988 state saves with 988
releases. V12 assigned mean probability 0.439 to the 106 positives and 0.400 to
the two negatives, but its Brier score was 0.321 versus a prevalence baseline
of 0.018. The ranking hint is not calibrated enough for control.

An evaluator-only validation resumed the earlier assisted trajectory immediately
before its first recorded milestone. Provenance was inherited as assisted and
is barred from strict training. Eight branches returned at depth 1. One
`LEFT,16` branch tested seven depth-1 paths plus 28 depth-2 paths without a
return; its best matched-NOOP L1 was 0.003007, beyond the 0.002 threshold. This
is the first explicit budget-scoped non-return and verifies that the collector
distinguishes an irreversible pixel outcome from normal animation. Every run
balanced saved states with releases, and both frozen model audits passed.

```text
experiments/lolo1-spatial-v13/shadow_evaluations/spatial-v13-bidirectional-probe-d2-b4-d1
experiments/lolo1-spatial-v13/shadow_evaluations/spatial-v13-room2-d291-bidirectional-d2-b4-d1
experiments/lolo1-spatial-v13/shadow_evaluations/spatial-v13-room2-entry-bidirectional-d2-b4-d1
experiments/lolo1-spatial-v13/shadow_evaluations/spatial-v13-room2-strict-bidirectional-d2-b4-12
