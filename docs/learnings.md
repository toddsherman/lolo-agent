...
- E6 (preregistered §15; report digest `82aa4af8…`, scorer byte-identical
  twice, validated against v336/v337 first): seam S3 — when the agent
  occupies a cell on or adjacent to a certified milestone under hold,
  deposit that position as an archive so E5's working restore key has a
  candidate to reach. A candidate, not a preference weight (§20 honored).

Result — **FAIL** (bits 1, 4 PASS; bits 2, 3 FAIL):

- **Bit 1 SUPPLY — PASS.** Identical d1–d7, first contested instant *is*
  the first divergence, archive geography columns 6–12 (44 vs 43). E5's
  shape holds.
- **Bit 2 CANDIDATE EXISTS — FAIL.** The gate ran 12 times and declined
  all 12 (`not_certified_adjacent`). **Zero deposits.**
- **Bit 3 OUTCOME — FAIL.** Neither arm collected `(12,11)`.
- V6 in vivo: v338 reproduces v336 state-for-state; the invariance chain
  is now **v333 ≡ v334 ≡ v336 ≡ v338**.

Mechanism (three events wide):

- At d16 the agent stood at `(12,10)`, distance 1. **Every conjunct of
  the deposit gate held** — configuration matched the hold signature,
  position certified, distance ≤ 1 — except objective liveness.
- The exploit hypothesis carrying `target_cells: [[12,11]]` had
  terminated `budget_exhausted` at d15. At d16 a `hold_configuration`
  hypothesis was active, whose payload is **empty by construction**. The
  exploit was re-activated at seq 82664 — **three events after** the d16
  commit at seq 82661.
- The pre-declared blind spot (§15.6(d), `recovery_distinct` filtering a
  same-frame deposit) was *not* the cause; it was never reached. Recorded
  rather than quietly dropped.

Classification:

- **Falsified at the measured gate**, and the narrowest failure yet:
  E5 showed the lever was attached to the wrong object; E6 attached it to
  the right object and found the object absent at the one instant it was
  needed.

Learning — the structural read across four failures:

- **The hypothesis layer and the incumbent planner run on different
  clocks.** Every Gate 4 failure is a misalignment between them, not a
  scoring defect: alive but no search to ride (§4.45); steering when it
  should not (§4.50); able only to rank candidates that exist (§4.51);
  dead at the decisive instant (here). The hypothesis layer's lifecycle
  is driven by its own budget counters; the moments that matter are
  generated by the incumbent's archive and stagnation machinery.
- This is **convergence, not whack-a-mole**: the failure point has moved
  from "no lever at all" to a three-event gap, and each experiment
  narrowed it. But four misalignments of the same class name the class as
  the finding.
- Deliberately NOT choosing the next point fix. Re-arming the budget
  would be tuning; widening the deposit radius is forbidden by §15.4 and
  would not have helped (`(12,7)`–`(12,9)` were already candidates and
  the d17 restore already took `(12,7)`).

Plan change:

- Commission a **design pass on hypothesis lifecycle**, not another seam:
  should hypothesis liveness be driven by the incumbent's events
  (approach, contested restore, milestone adjacency) rather than by
  decision-count budgets? And may a mechanism read certified cells from
  the record store rather than from a live hypothesis — a different
  intervention class that this experiment did not earn the right to
  choose.

Evidence:

- `docs/wp8-search-scheduling-design-2026-08-17.md` §15–§16
