...
class VerifiedNeuralAgent:
    ...
    def __init__(
        ...
        self.human_prior_navigation_detour_origin_control: Optional[
            Tuple[Action, int]
        ] = None
        self.human_prior_exhausted_navigation_detours: set[
            Tuple[str, Action, int]
        ] = set()
    ...
    def _filter_human_prior_navigation_detour_returns(
        ...
        for item in verified:
            ...
            if target_signature in blocked_signatures and not protected:
    ...
    def _human_prior_navigation_detour_progress_choice(
        self,
        verified: Sequence[Tuple[Any, ...]],
        analyses: Dict[int, Optional[HeartGoalAnalysis]],
        graph_signatures: Dict[int, Tuple[str, str]],
        target_position_visits: Dict[int, int],
    ) -> Tuple[Optional[Tuple[Any, ...]], int]:
        """Prefer route continuation before optional experiments.

        Positive distance progress comes first.  At a local plateau, a safe
        semantic egress is still more informative than an optional wait; rank
        those endpoints by semantic novelty before accepting another local
        regression.
        """

        if not self._human_prior_navigation_detour_active():
            return None, 0
        origin = self.human_prior_navigation_detour_origin_signature
        blocked_signatures = {
            signature
        ...
        progress = [
            item
            for item in verified
            if analyses.get(id(item[2])) is not None
            and analyses[id(item[2])].life_loss_penalty >= 0.0
            and self._human_prior_ordering_navigation_fields(
                analyses[id(item[2])]
            )["human_prior_navigation_ordering_reward"]
            > 0.0
            and graph_signatures.get(id(item[2]), ("", ""))[1]
            not in blocked_signatures
        ]
        positive_progress = bool(progress)
        if not progress:
            progress = [
                item
                for item in verified
                ...
                and item[1].path[0] != Action.NOOP
                and graph_signatures.get(id(item[2]), ("", ""))[1]
                and graph_signatures[id(item[2])][1]
                not in blocked_signatures
                and graph_signatures[id(item[2])][1]
                != graph_signatures[id(item[2])][0]
            ]
            if not progress:
                return None, 0
        return (
            max(
                ...
                key=lambda item: (
                    ...
                    target_position_visits.get(id(item[2]), 0) == 0,
                    -self.human_prior_graph_state_visits[
                        graph_signatures[id(item[2])][1]
                    ],
                    self._human_prior_ordering_navigation_fields(
                        analyses[id(item[2])]
                    )["human_prior_navigation_ordering_reward"],
    ...
    def _filter_human_prior_exhausted_navigation_detours(
        self,
        verified: List[Tuple[Any, ...]],
        analyses: Dict[int, Optional[HeartGoalAnalysis]],
        graph_signatures: Dict[int, Tuple[str, str]],
        ...
        """Remove unchanged controller egresses whose detour was exhausted."""

        if not self.human_prior_exhausted_navigation_detours:
            return verified, []
        blocked: List[Tuple[Any, ...]] = []
        for item in verified:
            state = item[2]
            analysis = analyses.get(id(state))
            source_signature = graph_signatures.get(
                id(state), ("", "")
            )[0]
            world_effect = world_contexts.get(id(state), ("", "", ""))[2]
            edge = (
            ...
            if (
                edge in self.human_prior_exhausted_navigation_detours
                and analysis is not None
                and analysis.milestone_reward <= 0.0
                and not world_effect
            ):
                blocked.append(item)
        if not blocked:
            return verified, []
        blocked_ids = {id(item[2]) for item in blocked}
        filtered = [
            item for item in verified if id(item[2]) not in blocked_ids
        ]
        return (filtered, blocked) if filtered else (verified, [])
    ...
    def reset(self, initial_frame: Optional[Frame] = None) -> Frame:
        ...
        self.human_prior_navigation_detour_previous_signature = ""
        self.human_prior_navigation_detour_origin_control = None
        self.human_prior_exhausted_navigation_detours.clear()
        self.pending_life_hazard_choice = None
    ...
    def seed_human_prior_episodic_memory(
        ...
        navigation_detour_previous_signature = ""
        navigation_detour_origin_control: Optional[Tuple[Action, int]] = None
        exhausted_navigation_detours: set[Tuple[str, Action, int]] = set()
        ...
        for event in events:
            if event.get("event") == "pixel_novel_room_started":
                ...
                navigation_detour_previous_signature = ""
                navigation_detour_origin_control = None
                exhausted_navigation_detours.clear()
                known_slots = {
                    (int(slot[0]), int(slot[1]))
                    for slot in (
            ...
            if event_type == "human_prior_navigation_detour_expired":
                exhausted_origin = str(
                    event.get("origin_graph_signature") or ""
                )
                exhausted_action = event.get("origin_action")
                exhausted_duration = event.get("origin_action_frames")
                if (
                    event.get("exhausted_edge_recorded", False)
                    and exhausted_origin
                    and exhausted_action is not None
                    and exhausted_duration is not None
                ):
                    try:
                        exhausted_navigation_detours.add(
                            (
                                exhausted_origin,
                                Action(str(exhausted_action)),
                                int(exhausted_duration),
                            )
                        )
                    except (TypeError, ValueError):
                        pass
            ...
            if event.get("human_prior_navigation_detour_armed", False):
                ...
                raw_detour_action = event.get(
                    "human_prior_navigation_detour_origin_action"
                ) or next(iter(event.get("path") or ()), None)
                raw_detour_duration = event.get(
                    "human_prior_navigation_detour_origin_action_frames"
                ) or next(iter(event.get("durations") or ()), None)
                if (
                    raw_detour_action is not None
                    and raw_detour_duration is not None
                ):
                    try:
                        navigation_detour_origin_control = (
                            Action(str(raw_detour_action)),
                            int(raw_detour_duration),
                        )
                    except (TypeError, ValueError):
                        navigation_detour_origin_control = None
            ...
            if event.get("human_prior_navigation_detour_active", False):
                navigation_detour_previous_signature = str(
                    event.get(
                        "human_prior_navigation_detour_previous_signature"
                    )
                    or (
                        source_signature
                        if source_signature != target_signature
                        else navigation_detour_previous_signature
                    )
                )
            if target_signature:
    ...
    def decide(self) -> Decision:
        ...
        if (
            ...
            expired_detour_edge = None
            if self.human_prior_navigation_detour_origin_control is not None:
                expired_detour_edge = (
                    self.human_prior_navigation_detour_origin_signature,
                    *self.human_prior_navigation_detour_origin_control,
                )
                self.human_prior_exhausted_navigation_detours.add(
                    expired_detour_edge
                )
            self._emit(
                ...
                origin_action=(
                    None
                    if expired_detour_edge is None
                    else expired_detour_edge[1]
                ),
                origin_action_frames=(
                    None
                    if expired_detour_edge is None
                    else expired_detour_edge[2]
                ),
                exhausted_edge_recorded=(
                    expired_detour_edge is not None
                ),
        ...
        try:
            ...
            ) = self._human_prior_navigation_detour_progress_choice(
                selection_verified,
                branch_goal_analyses,
                branch_goal_signatures,
                branch_goal_target_position_visits,
            )
            ...
            if anticipated_observation_choice is not None:
                ...
                selected_analysis = branch_goal_analyses[id(chosen[2])]
                detour_navigation_reward = float(
                    self._human_prior_ordering_navigation_fields(
                        selected_analysis
                    )["human_prior_navigation_ordering_reward"]
                )
                self._emit(
                    "human_prior_navigation_detour_progress_selected",
                    decision=self.decision_index + 1,
                    action=chosen[1].path[0],
                    action_frames=chosen[1].durations[0],
                    origin_graph_signature=(
                    ...
                    positive_navigation_alternatives=(
                        detour_progress_alternatives
                        if detour_navigation_reward > 0.0
                        else 0
                    ),
                    semantic_egress_alternatives=(
                        detour_progress_alternatives
                        if detour_navigation_reward <= 0.0
                        else 0
                    ),
            ...
            if navigation_detour_armed:
                ...
                self._emit(
                    ...
                    origin_action=plan.path[0],
                    origin_action_frames=plan.durations[0],
                ...
                self.last_navigation_change_decision = self.decision_index
            if (
                not navigation_detour_armed
                and self._human_prior_navigation_detour_active()
                and committed_goal_source_signature
                and committed_goal_target_signature
                and committed_goal_source_signature
                != committed_goal_target_signature
            ...
            self._emit(
                ...
                **committed_action_penalty_components,
                ...
                human_prior_navigation_detour_origin_action=(
                    None
                    if self.human_prior_navigation_detour_origin_control
                    is None
                    else self.human_prior_navigation_detour_origin_control[0]
                ),
                human_prior_navigation_detour_origin_action_frames=(
                    None
                    if self.human_prior_navigation_detour_origin_control
                    is None
                    else self.human_prior_navigation_detour_origin_control[1]
                ),
